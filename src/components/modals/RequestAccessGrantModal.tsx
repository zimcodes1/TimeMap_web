...
export default function RequestAccessGrantModal({
  ...
  useEffect(() => {
    if (selectedDepartmentId && !availableDepartments.some((d) => String(d.id) === String(selectedDepartmentId))) {
      setSelectedDepartmentId("");
    }
  }, [selectedFacultyId, availableDepartments, selectedDepartmentId]);

  // Determine if user has activated search or faculty selection
  const hasActivatedFilter = Boolean(searchQuery.trim() || selectedFacultyId || selectedDepartmentId);
  ...
  const filteredCourses = useMemo(() => {
    ...
    const targetFaculty = faculties.find((f) => String(f.id) === String(selectedFacultyId));
    const targetDept = departments.find((d) => String(d.id) === String(selectedDepartmentId));
  ...
  }, [externalCourses, searchQuery, selectedFacultyId, selectedDepartmentId, hasActivatedFilter, faculties, departments]);
            ...
            <div>
              ...
              <Select
                ...
                options={[
                  ...
                  ...availableDepartments.map((d) => ({ value: d.id, label: `${d.code} - ${d.name}` })),
                ]}
              />
            </div>
          )}
        ...
        {!hasActivatedFilter ? (
          <div className="p-4 rounded-lg bg-surface border border-border/80 text-center space-y-2">
            ...
            <Text variant="caption" color="muted" className="block">
              To keep course lists concise, available courses will be displayed once a faculty or search query is selected.
            </Text>
          </div>
        ) : filteredCourses.length === 0 ? (
          <div className="p-4 rounded-lg bg-surface border border-border/80 text-center space-y-1">
            ...
            {selectedCourse && (
              <div className="p-3 bg-raised/50 border border-border/50 rounded-lg text-xs space-y-1">
                <div className="flex items-center justify-between text-text-muted">
                  <span>Level: <strong className="text-text-main">{selectedCourse.level} Level</strong></span>
                  <span>Owner: <strong className="text-text-main">{selectedCourse.departmentName || selectedCourse.facultyName || "External"}</strong></span>
                </div>
                <p className="text-primary/90 font-medium">
                  Note: Access will be requested for students in your department matching <strong>{selectedCourse.level} Level</strong>.
                </p>
              </div>
            )}
          </div>
