...
interface CreateUserModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (data: Partial<User> & { scopeId?: string }) => void | Promise<void>;
...
export default function CreateUserModal({
  ...
  const isDeptAdmin = currentUser?.role === "admin" && currentAdminLevel === "department";
  const isFacultyAdmin = currentUser?.role === "admin" && currentAdminLevel === "faculty";
  const isSchoolAdmin = currentUser?.role === "admin" && currentAdminLevel === "school";
  ...
  const [adminLevel, setAdminLevel] = useState<AdminLevel | "">(initialData?.adminLevel || "");
  const [scopeId, setScopeId] = useState<string>(initialData?.departmentId || initialData?.adminScopeId || "");

  // Student specific fields
  const [level, setLevel] = useState<string>(initialData?.level ? String(initialData.level) : "");
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      let matchedScopeId =
        ...
        (initialData as { department_id?: string | number } | undefined)?.department_id ||
  ...
  // Generate level options up to department max level
  const availableLevels: Array<{ value: string; label: string; disabled?: boolean }> = [
    { value: "", label: "-- Select Academic Level --", disabled: true },
  ];
  for (let lvl = 100; lvl <= deptMaxLevel; lvl += 100) {
    availableLevels.push({ value: String(lvl), label: `${lvl} Level` });
  }

  // When scopeId changes for student, ensure level doesn't exceed deptMaxLevel
  useEffect(() => {
    if (role === "student" && level && Number(level) > deptMaxLevel) {
      setLevel("");
      setErrors((prev) => ({
        ...prev,
        level: `Selected level exceeds maximum level (${deptMaxLevel}L) for ${selectedDept?.name || "department"}.`,
      }));
    }
  }, [scopeId, deptMaxLevel, level, role, selectedDept?.name]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!identifier.trim()) {
      newErrors.identifier =
        role === "student" ? "Please enter student matric number." : "Please enter staff ID.";
    }
    ...
    if (role === "student") {
      ...
      } else if (Number(level) > deptMaxLevel) {
        newErrors.level = `Level ${level}L exceeds department max level (${deptMaxLevel}L).`;
      }
    }

    if (Object.keys(newErrors).length > 0) {
      setErrors(newErrors);
      setFormError("Form submission failed. Please complete all required fields correctly.");
      return;
    }
    ...
    try {
      ...
      if (typeof err === "object" && err !== null && "response" in err) {
        const res = (err as { response?: { data?: Record<string, string | string[]> } }).response;
        if (res?.data) {
          ...
          setErrors(apiErrors);
          setFormError("Failed to save user account. Please resolve the errors below.");
          return;
        }
      }
      setFormError(err instanceof Error ? err.message : "An unexpected error occurred during creation.");
    }
  };
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            <Text variant="caption" className="font-semibold mb-1 block text-text-muted">
              {role === "student"
                ? "Matric Number *"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            />
            {errors.name && (
              <span className="text-xs text-danger font-medium mt-1 block">{errors.name}</span>
            )}
          </div>
          <div>
            <Text variant="caption" className="font-semibold mb-1 block text-text-muted">
            ...
            />
            {errors.email && (
              <span className="text-xs text-danger font-medium mt-1 block">{errors.email}</span>
            )}
          </div>
        </div>
            ...
            <div>
              <Select
                ...
                onChange={(e) => {
                  ...
                  if (errors.scopeId) setErrors((prev) => ({ ...prev, scopeId: "" }));
                }}
                options={
                  !adminLevel
                    ? [{ value: "", label: "-- Select Admin Scope Level First --", disabled: true }]
                    : adminLevel === "school"
                    ? [
                        { value: "", label: "-- Select Scope School --", disabled: true },
                        ...scopedSchs.map((s) => ({ value: s.id, label: `${s.code} - ${s.name}` })),
                      ]
                    : adminLevel === "faculty"
                    ? [
                        { value: "", label: "-- Select Scope Faculty --", disabled: true },
                        ...scopedFacs.map((f) => ({ value: f.id, label: `${f.code} - ${f.name}` })),
                      ]
                    : [
                        { value: "", label: "-- Select Scope Department --", disabled: true },
                        ...scopedDepts.map((d) => ({ value: d.id, label: `${d.code} - ${d.name}` })),
                      ]
                }
              />
            </div>
        ...
        {(role === "student" || role === "lecturer") && (
          <div>
            <Select
              ...
              options={[
                ...(!isDeptAdmin ? [{ value: "", label: "-- Select Department --", disabled: true }] : []),
                ...scopedDepts.map((d) => ({
                  value: d.id,
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 items-start">
            <div>
              <Select
                ...
                helperText={
                  selectedDept
                    ? `Available levels for ${selectedDept.code}: 100L to ${deptMaxLevel}L`
