...
export default function AuthSplitLayout() {
	return (
		<div className="relative w-full bg-primary h-screen flex justify-between items-center gap-2 px-20 overflow-hidden">
			{/* Left Side */}
			<div className="w-4/7 relative flex justify-right items-end h-full">
				<div className="w-full h-fit flex gap-4 flex-col justify-start pb-10 items-start">
					{/* Logo badge */}
					<motion.span
						{...fadeUp(0)}
					...
					<motion.div {...fadeUp(0.1)}>
						<Text className="font-bold text-white text-5xl leading-tight">
							One platform for <br />
							<span className="text-white/50">every schedule,</span> every
							venue.
						</Text>
					</motion.div>
					...
					<motion.div {...fadeUp(0.2)}>
						<Text className="text-white/50 font-light text-base max-w-md">
							Eliminate timetable conflicts and venue clashes across NSUK —
					...
					<motion.div
						{...fadeUp(0.35)}
						className="relative z-10 mt-6 backdrop-blur-lg w-9/10 rounded-4xl bg-white/10 border border-white/15 p-6 flex flex-col gap-5"
					>
			...
			{/* Right panel */}
			<div className="w-1/3 bg-amber-50 h-full absolute right-20 -bottom-20 rounded-4xl"></div>
		</div>
	);
}
