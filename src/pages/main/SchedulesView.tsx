...
export default function SchedulesView({
	...
	const [activeTab, setActiveTab] = useState<
		"entries" | "sessions" | "past_sessions" | "exams"
	>("entries");
	...
	// Split sessions into active/upcoming vs past
	const today = new Date();
	const todayStr = `${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, "0")}-${String(today.getDate()).padStart(2, "0")}`;

	const activeSessions = sessions?.filter((s) => s.date >= todayStr);
	...
	const filteredEntries = entries?.filter((e) => {
		const matchesSearch =
			...
			(e.lecturerName &&
				e.lecturerName.toLowerCase().includes(searchQuery.toLowerCase()));
	...
	const filteredActiveSessions = activeSessions?.filter((s) => {
		...
		const matchesVenue = !venueFilter || s.venueName === venueFilter;
	...
	const isCurrentTabLoading =
		activeTab === "entries"
			? entriesLoading
			: activeTab === "sessions" || activeTab === "past_sessions"
				? sessionsLoading
				: examSittingsLoading;
		...
		<div className="space-y-6">
			...
			<div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
				<div>
					...
					<Text variant="body-sm" color="muted">
						Manage recurring lectures, dated sessions, and exam sittings with
						Conflict Detection Engine integration.
					</Text>
				</div>
			...
			<TabSwitcher
				tabs={[
					{
						id: "entries",
						label: "All Schedule Patterns",
						icon: CalendarIcon,
						count: entries?.length,
					},
					{
						id: "sessions",
						label: "Dated Sessions",
						icon: Layers,
				...
				viewMode === "grid" && activeTab === "entries" ? (
					<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
						{[...Array(6)].map((_, i) => (
							<Card
								key={i}
								className="p-4 space-y-3 border-l-4 border-l-border/50"
							>
							...
							columns={[
								...
								},
								{
									...
									accessor: (sess: LectureSession) => (
										<Badge
											variant={
												sess.status === "scheduled" ? "success" : "warning"
											}
										>
							...
							columns={[
								...
								},
								{
									...
									accessor: (sess: LectureSession) => (
										<Badge
											variant={
												sess.status === "shifted" ? "warning" : "default"
											}
										>
											{sess.status.toUpperCase()}
										</Badge>
								...
								},
								{
									...
									accessor: (sess: LectureSession) => {
										if (sess.reportStatus === "held") {
											return <Badge variant="success">Held</Badge>;
										}
										if (sess.reportStatus === "not_held") {
											return <Badge variant="danger">Not Held</Badge>;
										}
										return <Badge variant="warning">Undefined</Badge>;
									},
								},
							]}
