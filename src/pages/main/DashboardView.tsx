...
import { Building2, BookOpen, Clock, AlertTriangle, Loader2 } from "lucide-react";
import type {
  HoldRateAnalytics,
...
export default function DashboardView({
  ...
  const isAnyLoading = holdRateLoading || utilizationLoading || discrepanciesLoading || countsLoading;

  const pieData = [
    { name: "Approved", value: discrepancies?.summary?.byStatus?.approved ?? 0 },
    { name: "Rejected", value: discrepancies?.summary?.byStatus?.rejected ?? 0 },
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          {countsLoading ? (
            ...
            <div className="flex items-baseline justify-between">
              <Text variant="h4" weight="bold">
                {summaryCounts?.totalVenues ?? utilization?.summary?.totalVenues ?? 0} Venues
              </Text>
        ...
        <Card className="p-4 space-y-2 border-l-4 border-amber-500/20 border-l-amber-500">
          ...
          {countsLoading ? (
            ...
            <div className="flex items-baseline justify-between">
              <Text variant="h4" weight="bold">
                {summaryCounts?.pendingDiscrepancies ?? discrepancies?.summary?.byStatus?.pending ?? 0} Pending
              </Text>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="p-5 space-y-4 lg:col-span-1">
            ...
            {holdRateLoading ? (
              ...
              <>
                <Text variant="h6" weight="bold">
                  Lecture-Hold Rate ({holdRate?.summary?.holdRatePercentage ?? 0}%)
                </Text>
                <Text variant="caption" color="muted">
          ...
          <div className="h-64">
            {holdRateLoading ? (
            ...
            ) : holdRateBreakdown.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={holdRateBreakdown}>
              ...
              <div className="h-full flex items-center justify-center text-text-subtle text-xs border border-dashed border-border/50 rounded-xl">
                No lecture-hold reports recorded in this period.
              </div>
            )}
          </div>
        </Card>
        ...
        <Card className="p-5 space-y-4 lg:col-span-1">
          <div>
            {utilizationLoading ? (
              ...
              <>
                <Text variant="h6" weight="bold">
                  Venue Utilization ({utilization?.summary?.totalBookedHours ?? 0} Total Hours)
                </Text>
                <Text variant="caption" color="muted">
            ...
            {utilizationLoading ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={utilizationBreakdown}>
                  ...
                  <Tooltip />
                  <Bar dataKey="totalBookedHours" fill="#3b82f6" radius={[6, 6, 0, 0]} name="Booked Hours" />
                </BarChart>
              </ResponsiveContainer>
            ) : (
              <div className="h-full flex items-center justify-center text-text-subtle text-xs border border-dashed border-border/50 rounded-xl">
        ...
        <Card className="p-5 space-y-4 lg:col-span-1">
          <div>
            {discrepanciesLoading ? (
              ...
              <>
                <Text variant="h6" weight="bold">
                  Discrepancy Resolution Breakdown ({discrepancies?.summary?.totalDiscrepancies ?? 0} Total)
                </Text>
                <Text variant="caption" color="muted">
