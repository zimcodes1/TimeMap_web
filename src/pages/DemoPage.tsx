...
export function DemoPage() {
	const [activeTab, setActiveTab] = useState<
		"components" | "venues" | "timetable" | "discrepancies" | "reports"
	>("components");
	const [isModalOpen, setIsModalOpen] = useState(false);
	const [discrepancyList, setDiscrepancyList] =
		useState<DiscrepancyRequest[]>(mockDiscrepancies);
	...
	return (
		<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-8">
			{/* ─── Top Navigation Bar ────────────────────────────────────────────── */}
			<header className="sticky top-4 z-40 flex items-center justify-between p-4 rounded-2xl bg-white/80 backdrop-blur-md border border-gray-200 shadow-sm transition-all">
				...
				<div className="flex items-center gap-3">
					...
					<div className="hidden md:flex items-center gap-1.5 border-r border-gray-200 pr-3">
						<Button
							variant="ghost"
							size="sm"
							onClick={() =>
								toast.success("Action Completed!", {
									description: "Resource saved cleanly.",
								})
							}
						>
							Success Toast
						...
						<Button
							...
							size="sm"
							onClick={() =>
								toast.warning("Conflict Warning", {
					...
					<div className="flex items-center gap-2 pl-2 border-l border-gray-200">
						...
						<div className="hidden sm:block text-left text-xs">
							...
							<p className="text-gray-500 capitalize">
								{mockCurrentUser.adminLevel} Admin
							</p>
						</div>
					</div>
				</div>
			...
			<nav className="flex items-center gap-2 p-1.5 bg-gray-200/60 rounded-xl overflow-x-auto">
				<button
			...
			{activeTab === "components" && (
				<div className="space-y-8 animate-in fade-in duration-300">
					{/* Section: Multivariant Text Typography Component */}
					<Card>
						<CardHeader>
							<CardTitle>Multivariant Text Typography Component</CardTitle>
							<CardDescription>
								A polymorphic{" "}
								<code className="text-primary font-mono">Text</code> component
								supporting variants (h1-h6, lead, body, caption, overline,
								code), colors, weights, and alignment tied directly to CSS theme
								custom properties.
							</CardDescription>
						</CardHeader>
						<CardContent className="space-y-6">
							...
							<div className="space-y-2">
								...
								<div className="space-y-1.5 p-4 rounded-lg bg-surface-raised border border-border">
									<Text variant="h1">Heading 1 — Modern Timetable</Text>
									<Text variant="h2">Heading 2 — Venue Registry</Text>
									<Text variant="h3">Heading 3 — Lecture Conflicts</Text>
									<Text variant="h4">Heading 4 — Course Schedule Overview</Text>
									<Text variant="h5">Heading 5 — Section Details</Text>
									<Text variant="h6">Heading 6 — Sub-header Label</Text>
								</div>
							</div>

							{/* Body & Inline Variants */}
							<div className="space-y-2">
								<Text variant="overline" color="subtle">
									Body & Inline Variants
								</Text>
								<div className="space-y-2 p-4 rounded-lg bg-surface-raised border border-border">
									<Text variant="lead">
										Lead Paragraph: TimeMapper automatically detects schedule
										collisions across halls.
									</Text>
									<Text variant="body-lg">
										Body Large: Comprehensive departmental allocation system for
										modern universities.
									</Text>
									<Text variant="body">
										Body Default: Standard text used for descriptions,
										instructions, and list items.
									</Text>
									<Text variant="body-sm">
										Body Small: Auxiliary context text for footers or
										timestamps.
									</Text>
							...
							<div className="space-y-2">
								<Text variant="overline" color="subtle">
									Theme Color Variants (Tied to CSS)
								</Text>
								<div className="grid grid-cols-2 sm:grid-cols-4 gap-3 p-4 rounded-lg bg-surface-raised border border-border">
									<Text variant="body" color="default" weight="semibold">
										Default / Main
									</Text>
									<Text variant="body" color="muted" weight="semibold">
										Muted Text
									</Text>
									<Text variant="body" color="subtle" weight="semibold">
										Subtle Placeholder
									</Text>
									<Text variant="body" color="primary" weight="bold">
										Primary Brand
									</Text>
									<Text variant="body" color="danger" weight="bold">
										Danger Error
									</Text>
									<Text variant="body" color="success" weight="bold">
										Success Verified
									</Text>
									<Text variant="body" color="warning" weight="bold">
										Warning Alert
									</Text>
									<Text variant="body" color="info" weight="bold">
										Info Notice
									</Text>
								</div>
							</div>
					...
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Created with <code className="text-emerald-600">cva</code>,
								supporting variants (primary, secondary, outline, ghost, danger,
								warning, muted), sizes, loading states, and icon slots.
							</CardDescription>
						</CardHeader>
					...
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Contextual status indicators for schedule clashes, permissions,
								and report states.
							</CardDescription>
						</CardHeader>
						<CardContent className="flex flex-wrap items-center gap-3">
							...
							<Badge variant="danger" icon={<X className="h-3.5 w-3.5" />}>
								Hard Rejected
							</Badge>
							<Badge variant="info" icon={<Clock className="h-3.5 w-3.5" />}>
								Pending Review
							</Badge>
					...
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Form inputs & select dropdowns integrated with{" "}
								<code className="text-emerald-600">react-hook-form</code> and{" "}
								<code className="text-emerald-600">zod</code>.
							</CardDescription>
						</CardHeader>
						<CardContent>
				...
				<div className="space-y-6 animate-in fade-in duration-300">
					...
					<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
						{filteredVenues.map((venue) => (
							<Card
								key={venue.id}
								className="hover:border-emerald-500/50 transition-all shadow-sm"
							>
								<CardHeader>
								...
								<CardContent className="space-y-3">
									<div className="flex items-center gap-2 text-sm text-gray-700">
										...
										<span>
											Seating Capacity:{" "}
											<strong>{venue.capacity} students</strong>
										</span>
									</div>
								...
								<CardFooter className="justify-end border-t border-gray-100 pt-3">
									<Button
										variant="ghost"
										size="sm"
										onClick={() => setIsModalOpen(true)}
					...
					<Card>
						<CardHeader>
							<div className="flex items-center justify-between">
								<div>
									<CardTitle>
										Timetable Entries & Conflict Detection Engine
									</CardTitle>
									<CardDescription>
										Interval-based conflict detection prevents double-booking
										across venues and lecturers.
									</CardDescription>
								</div>
								<Badge
									variant="warning"
				...
				<div className="space-y-6 animate-in fade-in duration-300">
					<div className="flex items-center justify-between">
						<div>
							...
							<p className="text-xs text-gray-500">
								Hierarchical override routing for schedule shifts,
									...
									<div className="space-y-2">
										...
										<div className="flex flex-wrap items-center gap-4 text-xs text-gray-500">
											<span>
												Requested by:{" "}
												<strong>
													{disc.requestedBy} ({disc.requestedByRole})
												</strong>
											</span>
											{disc.requestedVenueName && (
												<span>
													New Venue: <strong>{disc.requestedVenueName}</strong>
												</span>
											)}
											{disc.requestedTimeSlot && (
												<span>
													Time Slot: <strong>{disc.requestedTimeSlot}</strong>
												</span>
											)}
										</div>
									</div>
									...
									{disc.status === "pending" ? (
										<div className="flex items-center gap-2 self-end md:self-center">
											<Button
												...
												onClick={() =>
													handleApproveDiscrepancy(disc.id, disc.courseCode)
												}
											>
												Approve
											</Button>
											<Button
												...
												onClick={() =>
													handleRejectDiscrepancy(disc.id, disc.courseCode)
												}
											>
												Reject
											</Button>
				...
				<div className="space-y-6 animate-in fade-in duration-300">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Mobile-first rep reporting logs with automatic 24-hour
