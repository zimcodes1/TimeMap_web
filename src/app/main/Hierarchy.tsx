...
export default function HierarchyContainer() {
	...
	const {
		...
		isRefetching: programsRefetching,
	...
	const createDepartmentMutation = useMutation({
		...
		onSuccess: () => {
			...
			queryClient.invalidateQueries({ queryKey: ["hierarchy", "programs"] });
		},
	...
	const handleCreateFaculty = (data: {
		schoolId: string;
		name: string;
		code: string;
	}) => {
	...
	const handleEditFaculty = (
		...
		updateFacultyMutation.mutate({
			id,
			payload: { school: schoolId, name, code },
		});
	};
	...
	const handleEditDepartment = (
		...
		updateDepartmentMutation.mutate({
			id,
			payload: { faculty: facultyId, name, code },
		});
	};
	...
	const handleConfirmDelete = () => {
		...
		else if (type === "Program") deleteProgramMutation.mutate(id);
	};

	const defaultDeptId = user?.adminScopeId || user?.departmentId;
	...
	return (
		<>
			...
			<EditProgramModal
				...
				onSubmit={(id, data) => updateProgramMutation.mutate({ id, data })}
